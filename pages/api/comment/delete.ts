...
export default async function handler(
  ...
  corsMiddleware(req, res, async () => {
    if (req.method === "DELETE") {
      ...
      const { commentId, chapterId } = req.query; // Lấy cả commentId và chapterId từ query
      ...
      try {
        const result = await deleteComment(commentId, chapterId); // Truyền cả commentId và chapterId vào hàm deleteComment
